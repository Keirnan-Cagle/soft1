...
public final class ABCDguesser2 {
    ...
    /**
     * Calculates the Error of the guess made with the exponents based off the
     * given mew value
     *
     * @param guess
     *            The guess made using the iteration of exponents
     *
     * @param mew
     *            The original value entered by the user
     *
     * @return The error calculated using the
     */
    private static double calculateError(double guess, double mew) {
        double error = 0.0;
    ...
    public static void main(String[] args) {
        ...
        /*
         * These variables are no longer needed because the iteration is being
         * done through for loops int a = 0; int b = 0; int c = 0; int d = 0;
         */
        ...
        for (int a = 0; a < expValues.length; a++) {
            for (int b = 0; b < expValues.length; b++) {
                for (int c = 0; c < expValues.length; c++) {
                    for (int d = 0; d < expValues.length; d++) {
                        guess = (Math.pow(w, expValues[a]))
                                * (Math.pow(x, expValues[b]))
                                * (Math.pow(y, expValues[c]))
                                * (Math.pow(z, expValues[d]));
                        calcError = calculateError(guess, mew);
                        if (calcError < acceptError) {
